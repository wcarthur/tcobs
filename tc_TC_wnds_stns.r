...
indir <- "/media/nHD2/TC_project_09_13/"

outdir <- "/media/nHD2/TC_project_09_13/new_TC_dist_600k/"
#N.B. Change non-readable characters [ ] by " " (stn name should be read as a single string)

fname <- paste(indir, "stn_TC_dist.txt", sep="")
bom_stns_wTC <- read.table(fname, skip=1, head=F)

#Read BoM datasets (half-hour) located in Dir 'datadir'
datadir <- "/media/nHD2/TC_project_09_13/BoM_datasets/half_hourly_gust/"
...
#   Write all stns found in BoM-provided datasets to R log file (to plot them in the OZ map):
    str1 <- paste(unique(stn_id),", ",stn_name$V1,",",unique(stn_lat),",",unique(stn_lon),",above, y",sep="")  
    print(str1)
...
#   loop tr' all cyclones:
    for(j in 1:length(cycl_mx_wsp) ){
       TC_gusts <- get_TC_gusts_per_cyclone(all_bomd[bdatas],unique(stn_ST),cycl_mx_wsp[[j]][1],cycl_mx_wsp[[j]][2],cycl_mx_wsp[[j]][3] )
       if(is.na(TC_gusts))next

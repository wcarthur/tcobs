...
stns_list <- "BoM_stns.txt"
BoM_stn_list <- read.csv(stns_list,head=T)
OZ_TCregion_stns <- with(BoM_stn_list,data.frame(stn_ID=Site,stn=Sitename,state=State,lat=Lat,lon=Lon,start=Start,endt=End) )
...
mx_d <- 600.0
keep_name <- c()
outf <- "stn_TC_dist.txt"
outf00 <- "No_TC_hit_stn.txt"
...
for(i in 1:length(OZ_TCregion_stns$stn) ){
  ...
  if (substring(OZ_TCregion_stns[i,7],4,7) == "    "){
    stn_close = 5000
  }else{
...
#         stop (13)
     ...
     if(dist_2stn <= mx_d){
            ...
            #Station is close enough, record station, distance and time of hit:
                    write.table(paste("[",as.character(stn_name),"] ",stn_ID," ",stn_state," ",signif((180.0/pi)*stn_lat,8)," ",signif((180.0/pi)*stn_lon,8),"  ",keep_name,"  ",round(dist_2stn,1),"  ",
                    OZ_TC_datab[k,2],"  ",OZ_TC_datab[k,5],sep=""),file=outf,append=TRUE,row.names=FALSE,col.names=FALSE,quote=FALSE)
            }else{
